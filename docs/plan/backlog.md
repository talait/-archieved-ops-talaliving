...
| S5 | **Three routes overflow at 390px for a reason inside the card body** — `/dashboard` (+279px), `/it/aturan-gaji` (+188px), `/inventory/penyesuaian` (+13px). The shared header cause is fixed (`CardHeader` `min-w-0`, M68); these three are each their own. Found by `npm run check:layout` and listed in its `KNOWN_OVERFLOW` so a **new** one still fails | Raised 2026-09-17 (M68, F100). Invisible by eye because the overflow pans inside `<main>` rather than the document — the page looks right until you drag it sideways. `/proyek/peti/label` is in the same list and is **not** a fault: A4 in real millimetres |
